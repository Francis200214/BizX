 ...
 *     @BizXCheckIntegerMax(
 *         max = 1000,
 *         error = @BizXCheckErrorMessage(code = 1006, message = "Value exceeds the maximum allowed")
 *     )
...
public @interface BizXCheckIntegerMax {
     ...
     * 指定的最大值。
     *
     * @return 最大值，默认为 {@link Integer#MAX_VALUE}
     */
    ...
    BizXCheckErrorMessage error() default @BizXCheckErrorMessage(
            code = VerificationErrorConstant.CheckIntegerMaxError.CODE,
            message = VerificationErrorConstant.CheckIntegerMaxError.MESSAGE);
